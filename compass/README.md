...
- Real: all state (goals, Life Wheel, confidence, journal, timeline) persists via localStorage, syncs to Neon Postgres if configured, and the app works offline once installed.
- Mocked: the coach's "understanding" is keyword-matching, not a language model. Voice input is a placeholder. Apple Health / Body Atelier / House of Harriet / Calendar integrations are stubbed as "not connected."
